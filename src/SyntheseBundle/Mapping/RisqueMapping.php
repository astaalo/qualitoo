<?php
namespace Orange\SyntheseBundle\Mapping;

use Orange\MainBundle\Entity\Risque;
use Orange\MainBundle\Mapping\BaseMapping;
...
class RisqueMapping extends BaseMapping {
	...
	private function putCauseInArch(&$data, $risque) {	
			foreach($risque->getCauseOfRisque() as $cor) {
				...
				if(!isset($data['cause'][$causeId])) {
					$data['cause'][$causeId] = array('name'=>$cor->getCause()->getLibelle(), 
	...
	public function calculGraviteByTop(&$arrData) {
		...
		foreach ($arrData as $key => $value) {
			$sId = $value['sId'];
			if(!isset($moyByRisqueStructure[$sId])) {
				$moyByRisqueStructure[$sId] = array(
						'menace'=>$value['menace'], 'libelle'=> $value['libelle'], 'count'=> 1 ,'somme'=> intval($value['gravite']), 'gravite' =>0
					);
			} else {
				$moyByRisqueStructure[$sId]['count'] = intval($moyByRisqueStructure[$sId]['count'])+1;
				$moyByRisqueStructure[$sId]['somme'] = intval($moyByRisqueStructure[$sId]['somme']) + intval($value['gravite']);
			}
		}
	...
	public function mapForTableauCriticiteAndGraviteByStructure(&$gravites, &$maturProb,$type,$criteria) {
		...
		$keys = array_column($maturProb, 'id');
		$keys = array_flip(array_map(function($value) { return (int)$value; }, $keys));
		foreach ($arrGraviteByStructure as $key => $tab) {
			if(isset($keys[$key])) {
				$maturProb[$keys[$key]]['gravite'] = $arrGraviteByStructure[$key]['gravite'];
				$maturProb[$keys[$key]]['criticite']=intval($arrGraviteByStructure[$key]['gravite']) * intval($maturProb[$keys[$key]]['probabilite']);
			}
		}
		return $maturProb;
	}
	...
	public function mapForTableauRisqueCriticite(&$data){
		...
		foreach ($data as $value) {
			$tableau[$value['id']] = array(
					'id'=> $value['id'], 'libelle'=> $value['libelle'] , 'gravite'=>0 , 'probabilite'=>0, 'maturite'=>0
				);
		}
